  ...
  describe 'class methods' do
    ...
    describe '::rank_by_items_sold' do
      it 'returns merchants ranked by total number of items sold' do
        ...
        expect(Merchant.rank_by_items_sold(3)).to eq([merchant_4, merchant_1, merchant_2])
        expect(Merchant.rank_by_items_sold(3)[0].items_sold_count).to eq(12)
        expect(Merchant.rank_by_items_sold(3)[1].items_sold_count).to eq(5)
        expect(Merchant.rank_by_items_sold(3)[2].items_sold_count).to eq(1)
      end
    end
  end

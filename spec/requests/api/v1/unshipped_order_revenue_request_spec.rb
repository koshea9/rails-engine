...
RSpec.describe 'Unshipped Order Revenue Request' do
  ...
  describe 'happy path' do
    it 'returns the total revenue for unshipped orders with quanitity limit' do
      ...
      get '/api/vi/revenue/unshipped?quantity=2'
      ...
      expect(unshipped_revenue).to have_key(:data)
      expect(unshipped_revenue[:data]).to be_a(Array)
      expect(unshipped_revenue[:data]).to have_key(:id)
      expect(unshipped_revenue[:data][:id]).to be_a(Integer)
      expect(unshipped_revenue[:data]).to have_key(:type)
      expect(unshipped_revenue[:data][:type]).to be_a(String)
      expect(unshipped_revenue[:data]).to have_key(:attributes)
      expect(unshipped_revenue[:data][:attributes]).to be_a(Hash)
      expect(unshipped_revenue[:data][:attributes]).to have_key(:potential_revenue)
      expect(unshipped_revenue[:data][:attributes][:potential_revenue]).to be_a(Float)
